...
class AllEventsBarChart extends React.Component {
  static propTypes = {
    data: arrayOf(object),
    xLabel: string,
    yLabel: string,
  ...
  computeTooltipProps = day => {
    const hoveredData = _.omit(_.find(this.props.data, {x: day}), 'x')
    ...
    return {
      ...
      content: `${day}: ${_.values(hoveredData).join(', ')}`
    }
  }
    ...
    return (
      <Wrapper className="barchart" hover={hover}>
        ...
        {chart !== LOADING &&
          hover &&
          hover.map((day, index) => (
            <Tooltip key={index} {...this.computeTooltipProps(day)} />
          ))}
      </Wrapper>
    )
  }
...
AllEventsBarChart.defaultProps = {
  chart: LOADING
}

export default withFauxDOM(
  withD3Renderer({updateOn: ['data', 'xLabel', 'yLabel']})(AllEventsBarChart)
)
