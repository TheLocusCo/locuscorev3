class Notification < ApplicationRecord
  ...
  scope :has_not_been_seen_by, -> (user_id) { where.not('viewed_users @> array[?]', user_id.to_i) }
  scope :check_authorization, -> (user_id) { User.is_admin?(user_id) ? all : where.not(n_type: 'admin') }
